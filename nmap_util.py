...
class Logger:
    ...
    FORMATTER = logging.Formatter(fmt=FORMAT_STR, datefmt=DATE_FMT)


    @staticmethod
    def get(name: str = __file__, level = logging.INFO, filename: Optional[str] = None) -> Optional[logging.Logger]:
...
logger = Logger.get(filename="nmap.log")


def main():
    freq_ports = Settings.ports_to_list()
    logger.info("count of freq ports: %s", len(freq_ports))
    logger.debug("freq_ports: %s", freq_ports)

    # 首先进行ping扫描
    logger.info("start ping scan ...")
    nm = nmap.PortScanner()
    nm.scan("", arguments="-sn -n -vvv -iL hosts.txt")
    
    logger.debug("ping cmd line: %s", nm.command_line())
    all_hosts: Set[str] = set(nm.all_hosts())
    logger.info("count of all hosts: %s", len(all_hosts))
    logger.debug("all hosts: %s", all_hosts)
    ping_alive_hosts: Set[str] = set([x for x in all_hosts if nm[x]['status']['state'] != "down"])
    logger.info("ping scan done. count of ping alive hosts: %s", len(ping_alive_hosts))
    logger.debug("ping alive hosts: %s", ping_alive_hosts)

    # 对ping检测不到的主机，对常用端口扫描
    to_test_hosts = all_hosts.difference(ping_alive_hosts)
    logger.debug("all host count %s, ping alive count %s, to tcp test host %s", len(all_hosts), len(ping_alive_hosts), len(to_test_hosts))
    tcp_alive_hosts: Set[str] = set()

    logger.info("start tcp freq ports scan for %s hosts ...", len(to_test_hosts))
    nm.scan("", arguments="-T4 -Pn -n -vvv --open -p {} {}".format(
        ",".join([str(x) for x in freq_ports]),
        " ".join(to_test_hosts)
    ))
    logger.debug("tcp freq port cmd line: %s", nm.command_line())
    for host in nm.all_hosts():
        ...
        for port, state in port_state.items():
            if "state" in state and state["state"] == "open":
                ...
                logger.debug("tcp alive: %s:%s", host, port)
    logger.info("tcp freq ports scan done. count of tcp alive hosts: %s", len(tcp_alive_hosts))
    logger.debug("tcp alive hosts: %s", tcp_alive_hosts)

    # 对存活主机进行全端口扫描
    logger.info("all host count %s, ping alive count %s, tcp alive count %s", len(all_hosts), len(ping_alive_hosts), len(tcp_alive_hosts))
    to_test_hosts = ping_alive_hosts.union(tcp_alive_hosts)
    logger.info("start tcp all ports scan for %s alive hosts ...", len(to_test_hosts))
    nm.scan("", arguments="-T4 -Pn -n -vvv --open -p- {}".format(" ".join(to_test_hosts)))
    logger.info("tcp all ports scan done.")
    logger.debug("tcp all ports cmd line: %s", nm.command_line())
    buffer = []
    for host in nm.all_hosts():
        port_state: Dict[int, str] = nm[host]["tcp"]
        for port, state in port_state.items():
...
def test():
    """logger test
    logger.debug("debug test %s", 1234567)
    logger.info("info test %s", 1234567)
    """
